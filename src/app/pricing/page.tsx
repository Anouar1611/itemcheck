      ...
      <div className="container mx-auto px-4 md:px-6 py-12 md:py-16">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {pricingTiers.map((tier) => (
            <Card key={tier.name} className={`flex flex-col shadow-lg bg-card ${tier.popular ? 'border-primary border-2 relative' : 'border-border'}`}>
              {tier.popular && (
              ...
              <CardHeader className="pb-4">
                <CardTitle className="text-2xl font-semibold text-card-foreground">{tier.name}</CardTitle>
                <CardDescription className="h-12 text-muted-foreground">{tier.description}</CardDescription>
                <div className="mt-4">
